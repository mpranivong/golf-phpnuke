...
function confirmNewUser($username, $user_email, $user_password, $user_password2, $random_num, $gfx_check) {
	global $stop, $EditedMessage, $sitename, $module_name, $minpass, $privatekey;
	include("header.php");
	include("config.php");
	...
	if (!$resp->is_valid) { //$test != "3651") {
		$stop = "<br><div align=\"center\" color=\"red\"><span class=\"option\"><strong><em>Invalid Verification Words!</em></span></font></div><br>";
	} else {
...
function new_user() {
	global $my_headlines, $module_name, $db, $gfx_chk, $user, $publickey;
	if (!is_user($user)) {
		...
		echo "<tr><td colspan='2'>Enter verification words separated with a space".recaptcha_get_html($publickey)."</td></tr>";
