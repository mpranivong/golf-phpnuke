...
function AddLink() {
	...
	."Confirm email: <input type=\"text\" name=\"checker\" size=\"30\" maxlength=\"60\"><br><br>"
...
function Add($title, $url, $auth_name, $cat, $description, $email, $checker) {
	...
	if ($email != $checker) {
		...
		echo "<br>";
		OpenTable();
		echo "<center><b>Please confirm email address</b><br><br>"
		.""._GOBACK."";
		CloseTable();
		include("footer.php");
	} else if ($numrows>0) {
		...
		if ($num_new == 0) {
			$sql = "insert into ".$prefix."_links_newlink values (NULL, '$cat[0]', '$cat[1]', '$title', '$url', '$description', '$auth_name', '$email', '$submitter',0)";
			echo $sql;
			$db->sql_query($sql);
		}
		include("header.php");
...
switch($l_op) {
	...
	case "Add":
	Add($title, $url, $auth_name, $cat, $description, $email, $checker);
	break;
